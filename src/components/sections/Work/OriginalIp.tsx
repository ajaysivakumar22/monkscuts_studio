...
export function OriginalIp() {
  return (
    <Section id="original-ip" className="bg-[var(--color-surface)]">
      <Container>
        ...
        <div className="mt-16 grid gap-px overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-line)] bg-[var(--color-line)] md:grid-cols-2">
          {originalWorks.map((work) => (
            <Reveal key={work.title} className="flex flex-col justify-between gap-10 bg-[var(--color-surface)] p-10">
              <div>
                <p className="font-mono text-[var(--text-label)] uppercase tracking-[0.15em] text-[var(--color-monk)]">
                  {work.category}
                </p>
                <h3 className="mt-4 text-[var(--text-h3)] font-semibold tracking-tight text-[var(--color-text)]">
                  {work.title}
                </h3>
              ...
              <span className="w-fit border border-[var(--color-line)] px-4 py-2 font-mono text-[var(--text-label)] uppercase tracking-[0.15em] text-[var(--color-dim)]">
                {work.status}
              </span>
            </Reveal>
          ))}
