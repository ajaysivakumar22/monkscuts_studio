...
export function Hero() {
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      ...
      gsap.set("[data-hero-face]", { opacity: 0.65, scale: 1, y: 0, x: 0 });
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div
        ref={auraRef}
        aria-hidden="true"
        className="pointer-events-none absolute right-[10%] top-[15%] -z-10 h-[45vw] w-[45vw] max-h-[650px] max-w-[650px] rounded-full blur-[120px] opacity-70"
        style={{
          background: "radial-gradient(circle, rgba(200,122,0,0.15) 0%, rgba(200,138,0,0.04) 50%, transparent 70%)",
        }}
      />
      ...
      {/* ══════════════════════════════════════════════════════════════
          ...
          Positioned slightly UP (top-[42%] -translate-y-[50%]) for vertical
          balance with MONKCUTS / STUDIO text block.
          Zero red tint / glow around the artwork.
      ══════════════════════════════════════════════════════════════ */}
      <div
        className="pointer-events-none absolute right-[2%] sm:right-[4%] md:right-[6%] lg:right-[9%] xl:right-[11%] top-[38%] -translate-y-[46%] select-none"
        style={{ zIndex: 1 }}
        aria-hidden="true"
      >
        <MonkFace
      ...
      {/* ── CONTENT — sits in the LEFT 50% on desktop ── */}
      <Container className="relative z-10 my-auto w-full">
        {/* Eyebrow */}
        <div data-hero-eyebrow className="flex items-center gap-3">
          <span className="relative flex h-2.5 w-2.5">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--color-monk-face)] opacity-75" />
            <span className="relative inline-flex h-2 w-2 rounded-full bg-[var(--color-monk-face)]" />
          </span>
          <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--color-monk-face)]">
            {hero.eyebrow}
          </span>
        </div>
        ...
        {/* Subtext & CTAs */}
        <div className="mt-10 md:mt-12 max-w-[58%] sm:max-w-[54%] md:max-w-[48%] lg:max-w-[44%] space-y-8">
          <p
            data-hero-sub
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div data-hero-cta>
              <Button
                ...
                variant="primary"
                className="group w-full sm:w-auto justify-center font-bold tracking-[0.12em] shadow-[0_0_24px_rgba(200,122,0,0.22)] hover:shadow-[0_0_36px_rgba(200,122,0,0.42)] hover:scale-[1.02] transition-all duration-300"
              >
        ...
        >
          ...
          <a
            href="#studio"
            className="group inline-flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.2em] text-[var(--color-dim)] transition-colors duration-300 hover:text-[var(--color-monk-face)]"
          >
            <span>Explore Studio</span>
            <span className="flex h-7 w-7 items-center justify-center rounded-full border border-[var(--color-line-subtle)] transition-all duration-300 group-hover:border-[var(--color-monk-face)] group-hover:bg-[rgba(200,122,0,0.1)]">
              <ArrowDown className="h-3.5 w-3.5 text-[var(--color-monk-face)] transition-transform group-hover:translate-y-0.5" aria-hidden="true" />
            </span>
          </a>
        </div>
      </Container>
